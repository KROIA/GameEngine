...
Controller::~Controller()
{
    //EngineMisc::__deleteLater(TYPE_TAG_CONTROLLER,m_ID);
...
void Controller::applyMoveList()
{
   // m_lastPos = m_pos;
    for(unsigned int a=0; a<m_moveList.size(); a++)
    {
        ...
        else if(m_pos.x+m_size.x+m_moveList[a].x <= EngineMisc::mapSize.x)
        {
          //  m_lastPos.x = m_pos.x;
            m_pos.x+=m_moveList[a].x;
        }else
        {
            m_pos.x = EngineMisc::mapSize.x-m_size.x;
            m_velocity.direction.x=0;
        }
        ...
        else if(m_pos.y+m_size.y+m_moveList[a].y <= EngineMisc::mapSize.y)
        {
           // m_lastPos.y = m_pos.y;
            m_pos.y+=m_moveList[a].y;
        }else
        {
            m_pos.y = EngineMisc::mapSize.y-m_size.y;
            m_velocity.direction.y=0;
        }*/
    }
    m_moveList.clear();
